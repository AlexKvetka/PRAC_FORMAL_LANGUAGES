...
Grammar removeEpsilon(const Grammar& grammar) {
  ...
  for (unsigned symbol_number = 0;
       symbol_number < result_grammar.symbols.size(); ++symbol_number) {
...
int classifyRuleChomskyToGreybuh(const Rule& rule,
                                 const string& starting_symbol) {
  if (rule.from == starting_symbol && rule.to.size() == 1 &&
      rule.to[0] == "epsilon") {
    return 0;
  }
  ...
  if (rule.to.size() == 1 &&
      (!isAlphabetSymbol(rule.to[0]) || (rule.to[0] == "epsilon"))) {
...
Grammar chomskyToGreybuh(const Grammar& grammar) {
  ...
  for (unsigned int i = 0; i < grammar.symbols.size(); ++i) {
    result_grammar.addRule(
        {grammar.symbols[i] + "\\" + grammar.symbols[i], {"epsilon"}});
  }
  ...
  for (unsigned rule1_number = 0; rule1_number < grammar.rules.size();
       ++rule1_number) {
    Rule rule1 = grammar.rules[rule1_number];
    if (classifyRuleChomskyToGreybuh(rule1, grammar.starting_symbol) != 1) {
      continue;
    }
    for (unsigned rule2_number = 0; rule2_number < grammar.rules.size();
         ++rule2_number) {
      Rule rule2 = grammar.rules[rule2_number];
      if (classifyRuleChomskyToGreybuh(rule2, grammar.starting_symbol) != 2) {
        continue;
      }
      for (unsigned B_symbol_number = 0;
           B_symbol_number < grammar.symbols.size(); ++B_symbol_number) {
        ...
        result_grammar.addRule({A_symbol + "\\" + B_symbol,
                                {e_symbol, E_symbol + "\\" + D_symbol,
                                 C_symbol + "\\" + B_symbol}});
      }
    }
  }
