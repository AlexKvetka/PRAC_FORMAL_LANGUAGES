...
bool operator==(const Situation& s1, const Situation& s2) {
  return s1.rule == s2.rule &&
         s1.deduced_prefix_length == s2.deduced_prefix_length &&
...
bool EarleyAlgorithm::complete_(int d_number, const string& s) {
  ...
  for (const auto& situation_j : D_situations_[d_number]) {
    if (situation_j.position_in_rule !=
        static_cast<int>(situation_j.rule.to.size())) {
      continue;
    }
    for (const auto& situation_k :
         D_situations_[situation_j.deduced_prefix_length]) {
      if (situation_k.position_in_rule ==
          static_cast<int>(situation_k.rule.to.size())) {
        continue;
      }
      if (situation_k.rule.to[situation_k.position_in_rule] !=
          situation_j.rule.from) {
        continue;
      }
...
void EarleyAlgorithm::finalize_() { D_situations_.clear(); }
...
bool EarleyAlgorithm::isRecognized(const Grammar& grammar, const string& s) {
  ...
  bool answer =
      std::find(D_situations_[s.size()].begin(), D_situations_[s.size()].end(),
